...
| 配置管理（启动首屏） | 角色编辑 |
| --- | --- |
| ![配置管理](docs/screenshots/01-配置管理.png) | ![角色编辑](docs/screenshots/02-角色编辑.png) |

| 循环时间轴 | 灵动岛（展开后的四个按钮） |
| --- | --- |
| ![循环时间轴](docs/screenshots/03-循环时间轴.png) | ![灵动岛](docs/screenshots/04-灵动岛展开.png) |

| 快捷配置助手 | 伤害占比 |
| --- | --- |
| ![快捷配置助手](docs/screenshots/05-快捷配置助手.png) | ![伤害占比](docs/screenshots/06-伤害占比.png) |

| 面板时序解析 | 计算公式总览 |
| --- | --- |
| ![面板时序解析](docs/screenshots/07-面板时序解析.png) | ![计算公式总览](docs/screenshots/08-计算公式总览.png) |

> 截图由 `exe_src/tools/dev_preview.py` 渲染，可随时重新生成。
...
常用快捷键：`Ctrl+S` 保存、`Ctrl+Q` 保存并退出、`Ctrl+T` 快速拉表、`Ctrl+,` 全局设置、`Esc` 收起灵动岛。
